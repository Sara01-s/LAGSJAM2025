...
public class AdaptiNode : MonoBehaviour
{
    ...
    // DEBUG //
    [Header("Debug")]
    [SerializeField] private bool measure_count = false;
    ...
    //private float pitch;
    [HideInInspector] public AudioMixerGroup bus;
    ...
    // Sequence Variables //
    private bool can_sequence = false;
    private string sequence_track_name;
    [HideInInspector] public UnityEvent endMusicSignal;
    ...
    {
        can_sequence = false; // for sequence
        volume = targetVolume;
        ...
        if (intro_clip != null)
        {
            ...
            if (duration == 0.0f)
            {
                intro_source.audio_source.volume = targetVolume;
            }
    ...
    public void mute_all_layer(float fade_time = 1.5f, bool fade_type = true)
    {
        // Get Loop Source Index
        int loop_key = get_loop_index();
        if (loop_key == -1)
        ...
        if (fade_type)
        {
            if (fade_time != 0.0f)
            {
                for (int i = 0; i < loops_sources[loop_key].Length; i++)
                {
                    loops_sources[loop_key][i].StartFade(fade_time, 0.0f, false, false);
                }
            }
            else
            {
                for (int i = 0; i < loops_sources[loop_key].Length; i++)
                {
                    loops_sources[loop_key][i].audio_source.volume = 0.0f;
                    loops_sources[loop_key][i].stop_coroutine();
                }
            }
        ...
        else
        {
            if (fade_time != 0.0f)
            {
                for (int i = 0; i < loops_sources[loop_key].Length; i++)
                {
                    loops_sources[loop_key][i].StartFade(fade_time, volume, true, false);
                }
            }
            else
            {
                for (int i = 0; i < loops_sources[loop_key].Length; i++)
                {
                    loops_sources[loop_key][i].audio_source.volume = volume;
                    loops_sources[loop_key][i].stop_coroutine();
                }
            }
        }
    ...
    public void change_loop(int loop_index, float fade_out = 0.5f, float fade_in = 0.5f)
    {
        ...
        if (current_playback == intro_source && current_playback == outro_source)
        {
            AudioManager.print("log", "No loop playing");
            return;
        }
    ...
    private void change_outro(int loop_index, float fade_out, float fade_in)
    {
        ...
        {
            AudioManager.print("warning", "No outro file, use stop_music");
            return;
            //on_stop()
        }
    ...
    public void on_stop(float fade_out = 1.5f)
    {
        ...
        {
            can_sequence = false;
            AudioManager.Instance.playMusic(sequence_track_name);
            endMusicSignal.Invoke();
        }
    }

    public void set_sequence(string track_name)
    {
        can_sequence = true;
